 ...
 */
package eu.archivesportaleurope.portlet.spring;
...
public class AnnotationMethodHandlerAdapter extends PortletContentGenerator
	...
	public static final String IMPLICIT_MODEL_SESSION_ATTRIBUTE = AnnotationMethodHandlerAdapter.class
			.getName() + ".IMPLICIT_MODEL";
	 ...
	 * <p>
	 * In many cases, the PortletSession reference itself is a safe mutex as
	 * well, since it will always be the same object reference for the same
	 * active logical session. However, this is not guaranteed across different
	 * servlet containers; the only 100% safe way is a session mutex.
	 * 
	...
	/**
	 * Set a custom WebArgumentResolver to use for special method parameter
	 * types. Such a custom WebArgumentResolver will kick in first, having a
	 * chance to resolve an argument value before the standard argument handling
	 * kicks in.
	 */
	 ...
	 * types. Such a custom ModelAndViewResolver will kick in first, having a
	 * chance to resolve an return value before the standard ModelAndView
	 * handling kicks in.
	 */
	 ...
	 * return types. Any such custom ModelAndViewResolver will kick in first,
	 * having a chance to resolve an return value before the standard
	 * ModelAndView handling kicks in.
	 */
	...
	public void handleAction(ActionRequest request, ActionResponse response,
		...
		if (returnValue != null) {
			throw new IllegalStateException(
					"Invalid action method return value: " + returnValue);
		}
	}
	...
	public void handleEvent(EventRequest request, EventResponse response,
		...
		if (returnValue != null) {
			throw new IllegalStateException(
					"Invalid event method return value: " + returnValue);
		}
	}
	...
	protected ModelAndView doHandle(PortletRequest request,
		...
		if (response instanceof MimeResponse) {
			...
			if (response instanceof RenderResponse) {
				implicitModel = retrieveImplicitModel((RenderRequest) request);
			}
			if (handler.getClass().getAnnotation(SessionAttributes.class) != null) {
				// Always prevent caching in case of session attribute
				// management.
	...
	private ModelAndView invokeHandlerMethod(PortletRequest request,
			PortletResponse response, Object handler,
		...
		PortletHandlerMethodInvoker methodInvoker = new PortletHandlerMethodInvoker(
				methodResolver);
		...
		if (response instanceof StateAwareResponse && !implicitModel.isEmpty()) {
			...
			persistImplicitModel(request, stateResponse,implicitModel);
		}
	...
	private PortletHandlerMethodResolver getMethodResolver(Object handler) {
		...
		PortletHandlerMethodResolver resolver = this.methodResolverCache
				.get(handlerClass);
		if (resolver == null) {
	 ...
	 */
	private static class PortletHandlerMethodResolver extends
		...
		protected boolean isHandlerMethod(Method method) {
			...
			ActionMapping actionMapping = AnnotationUtils.findAnnotation(
					method, ActionMapping.class);
			RenderMapping renderMapping = AnnotationUtils.findAnnotation(
					method, RenderMapping.class);
			ResourceMapping resourceMapping = AnnotationUtils.findAnnotation(
					method, ResourceMapping.class);
			EventMapping eventMapping = AnnotationUtils.findAnnotation(method,
					EventMapping.class);
			RequestMapping requestMapping = AnnotationUtils.findAnnotation(
					method, RequestMapping.class);
			if (actionMapping != null) {
				mappingInfo.initPhaseMapping(PortletRequest.ACTION_PHASE,
						actionMapping.value(), actionMapping.params());
			}
			if (renderMapping != null) {
				mappingInfo.initPhaseMapping(PortletRequest.RENDER_PHASE,
						renderMapping.value(), renderMapping.params());
			}
			if (resourceMapping != null) {
				mappingInfo.initPhaseMapping(PortletRequest.RESOURCE_PHASE,
						resourceMapping.value(), new String[0]);
			}
			if (eventMapping != null) {
				mappingInfo.initPhaseMapping(PortletRequest.EVENT_PHASE,
						eventMapping.value(), new String[0]);
			}
			if (requestMapping != null) {
				mappingInfo.initStandardMapping(requestMapping.value(),
						requestMapping.method(), requestMapping.params(),
		...
		public Method resolveHandlerMethod(PortletRequest request)
			...
			for (Method handlerMethod : getHandlerMethods()) {
				RequestMappingInfo mappingInfo = this.mappings
						.get(handlerMethod);
				if (mappingInfo.match(request)) {
					Method oldMappedMethod = targetHandlerMethods.put(
							mappingInfo, handlerMethod);
					if (oldMappedMethod != null
							&& oldMappedMethod != handlerMethod) {
						throw new IllegalStateException(
								"Ambiguous handler methods mapped for portlet mode '"
			...
			if (!targetHandlerMethods.isEmpty()) {
				if (targetHandlerMethods.size() == 1) {
					...
					for (RequestMappingInfo mapping : targetHandlerMethods
							.keySet()) {
				...
				throw new NoHandlerFoundException(
						"No matching handler method found for portlet request",
						request);
			}
		}
		...
		private String determineDefaultPhase(Method handlerMethod) {
			...
			for (Class<?> argType : handlerMethod.getParameterTypes()) {
				if (ActionRequest.class.isAssignableFrom(argType)
						|| ActionResponse.class.isAssignableFrom(argType)
						|| InputStream.class.isAssignableFrom(argType)
						|| Reader.class.isAssignableFrom(argType)) {
					return PortletRequest.ACTION_PHASE;
				} else if (RenderRequest.class.isAssignableFrom(argType)
						|| RenderResponse.class.isAssignableFrom(argType)
						|| OutputStream.class.isAssignableFrom(argType)
						|| Writer.class.isAssignableFrom(argType)) {
					return PortletRequest.RENDER_PHASE;
	...
	private class PortletHandlerMethodInvoker extends HandlerMethodInvoker {
		...
		protected void raiseMissingParameterException(String paramName,
			...
			throw new MissingPortletRequestParameterException(paramName,
		...
		protected WebDataBinder createBinder(NativeWebRequest webRequest,
			...
			return AnnotationMethodHandlerAdapter.this.createBinder(
					webRequest.getNativeRequest(PortletRequest.class), target,
					objectName);
		}
		...
		protected void doBind(WebDataBinder binder, NativeWebRequest webRequest)
			...
			portletBinder.bind(webRequest
					.getNativeRequest(PortletRequest.class));
		}
		...
		protected Object resolveCookieValue(String cookieName, Class paramType,
			...
			PortletRequest portletRequest = webRequest
					.getNativeRequest(PortletRequest.class);
			Cookie cookieValue = PortletUtils.getCookie(portletRequest,
					cookieName);
		...
		protected Object resolveStandardArgument(Class<?> parameterType,
			...
			PortletRequest request = webRequest
					.getNativeRequest(PortletRequest.class);
			PortletResponse response = webRequest
					.getNativeResponse(PortletResponse.class);
			...
			if (PortletRequest.class.isAssignableFrom(parameterType)
				...
				if (!(request instanceof ClientDataRequest)) {
					throw new IllegalStateException(
							"InputStream can only get obtained for Action/ResourceRequest");
				}
				return ((ClientDataRequest) request).getPortletInputStream();
			} else if (Reader.class.isAssignableFrom(parameterType)) {
				if (!(request instanceof ClientDataRequest)) {
					throw new IllegalStateException(
							"Reader can only get obtained for Action/ResourceRequest");
				}
				return ((ClientDataRequest) request).getReader();
			} else if (OutputStream.class.isAssignableFrom(parameterType)) {
				if (!(response instanceof MimeResponse)) {
					throw new IllegalStateException(
							"OutputStream can only get obtained for Render/ResourceResponse");
				}
				return ((MimeResponse) response).getPortletOutputStream();
			} else if (Writer.class.isAssignableFrom(parameterType)) {
				if (!(response instanceof MimeResponse)) {
					throw new IllegalStateException(
							"Writer can only get obtained for Render/ResourceResponse");
				}
				return ((MimeResponse) response).getWriter();
			} else if (Event.class.equals(parameterType)) {
				if (!(request instanceof EventRequest)) {
					throw new IllegalStateException(
							"Event can only get obtained from EventRequest");
				}
	...
	private static class RequestMappingInfo {
		...
		public void initStandardMapping(String[] modes,
				RequestMethod[] methods, String[] params, String[] headers) {
		...
		public void initPhaseMapping(String phase, String value, String[] params) {
			if (this.phase != null) {
				throw new IllegalStateException(
						"Invalid mapping - more than one phase specified: '"
								+ this.phase + "', '" + phase + "'");
			}
			this.phase = phase;
			this.value = value;
		...
		public boolean match(PortletRequest request) {
			if (!this.modes.isEmpty()
					&& !this.modes.contains(request.getPortletMode())) {
				return false;
			}
			if (StringUtils.hasLength(this.phase)
					&& !this.phase.equals(request
			...
			if (StringUtils.hasLength(this.value)) {
				if (this.phase.equals(PortletRequest.ACTION_PHASE)
						&& !this.value.equals(request
								.getParameter(ActionRequest.ACTION_NAME))) {
					return false;
				} else if (this.phase.equals(PortletRequest.RENDER_PHASE)
						&& !(new WindowState(this.value)).equals(request
								.getWindowState())) {
					return false;
				} else if (this.phase.equals(PortletRequest.RESOURCE_PHASE)
						&& !this.value.equals(((ResourceRequest) request)
								.getResourceID())) {
					return false;
				} else if (this.phase.equals(PortletRequest.EVENT_PHASE)) {
					Event event = ((EventRequest) request).getEvent();
					if (!this.value.equals(event.getName())
							&& !this.value.equals(event.getQName().toString())) {
						return false;
					}
				}
			}
			return PortletAnnotationMappingUtils.checkRequestMethod(
					this.methods, request)
					&& PortletAnnotationMappingUtils.checkParameters(
							this.params, request)
					&& PortletAnnotationMappingUtils.checkHeaders(this.headers,
							request);
		}

		public boolean isBetterMatchThan(RequestMappingInfo other) {
			return ((!this.modes.isEmpty() && other.modes.isEmpty())
					|| (StringUtils.hasLength(this.phase) && !StringUtils
							.hasLength(other.phase))
					|| (StringUtils.hasLength(this.value) && !StringUtils
							.hasLength(other.value))
		...
		@Override
		public int hashCode() {
			return (ObjectUtils.nullSafeHashCode(this.modes) * 29 + this.phase
					.hashCode());
		}
	}
	/**
	 * Overwrite this method to forbid Spring to write objects to the session between action/event phase
	 * 
	 * @param request
	 * @param stateResponse
	 * @param implicitModel
	...
	protected void persistImplicitModel(PortletRequest request, StateAwareResponse stateResponse, ExtendedModelMap implicitModel){
		Map<?, ?> modelToStore = implicitModel;
		try {
			stateResponse.setRenderParameter(
					IMPLICIT_MODEL_RENDER_PARAMETER,
					Boolean.TRUE.toString());
			if (stateResponse instanceof EventResponse) {
				// Update the existing model, if any, when responding to an
				// event -
				// whereas we're replacing the model in case of an action
				// response.
				Map existingModel = (Map) request.getPortletSession()
			...
			request.getPortletSession().setAttribute(
					IMPLICIT_MODEL_SESSION_ATTRIBUTE, modelToStore);
		} catch (IllegalStateException ex) {
			// Probably sendRedirect called... no need to expose model to
			// render phase.
		}		
	}
	
	/**
	 * Overwrite this method to forbid Spring to read objects from the session in the render phase.
	 * @param request
	 * @return
	 */
	protected ExtendedModelMap retrieveImplicitModel(RenderRequest request){
		ExtendedModelMap implicitModel = null;
		PortletSession session = request.getPortletSession(false);
		if (session != null) {
			if (request.getParameter(IMPLICIT_MODEL_RENDER_PARAMETER) != null) {
				implicitModel = (ExtendedModelMap) session
						.getAttribute(IMPLICIT_MODEL_SESSION_ATTRIBUTE);
			} else {
