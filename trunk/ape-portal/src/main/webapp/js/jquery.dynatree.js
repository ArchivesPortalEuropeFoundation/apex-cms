...
//		this.tree.logDebug("%s.addChild(%o, %o)", this, obj, beforeNode);
	...
	_insertChildBefore: function(dtnode, index) {
		/**
		 * Internal function to add one single DynatreeNode as a child.
		 *
		 */
...
//			throw "<beforeNode> must be another child of <this>";
		...
		if( index >= 0 ) {
			this.childList.splice(index, 0, dtnode);
		} else {
			// Append node
			this.childList.push(dtnode);
		}
...
//			dtnode._fixSelectionState();
	...
	insertChildBefore: function(obj, index) {
		/**
		 * Add a node object as child.
		 *
		 * This should be the only place, where a DynaTreeNode is constructed!
...
//		this.tree.logDebug("%s.addChild(%o, %o)", this, obj, beforeNode);
		if(typeof(obj) == "string"){
			...
			return this._insertChildBefore(obj, index);
		}
		...
		for (var i=0, l=obj.length; i<l; i++) {
			var data = obj[i];
			var dtnode = this._insertChildBefore(new DynaTreeNode(this, this.tree, data), i);
			if( !tnFirst ){
				tnFirst = dtnode;
			}
			// Add child nodes recursively
...
//				self.tree.logDebug("appendAjax().success");
	...
	insertBeforeAjaxWithoutRemove: function(ajaxOptions, dtnode) {
		var self = this;
		//this.removeChildren(false, true);
		this.setLazyNodeStatus(DTNodeStatus_Loading);
		// Debug feature: force a delay, to simulate slow loading...
...
//				self.tree.logDebug("appendAjax().success");
	...
	appendAjaxWithoutRemove: function(ajaxOptions) {
		var self = this;
		//this.removeChildren(false, true);
		this.setLazyNodeStatus(DTNodeStatus_Loading);
		// Debug feature: force a delay, to simulate slow loading...
